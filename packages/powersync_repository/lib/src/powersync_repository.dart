...
class SupabaseConnector extends PowerSyncBackendConnector {
  ...
  Future<PowerSyncCredentials?> fetchCredentials() async {
    ...
    return PowerSyncCredentials(
      endpoint: shared.Environment.POWERSYNC_URL,
      // endpoint: env(Environment.powerSyncUrl),
      token: token,
      userId: userId,
      expiresAt: expiresAt,
    );
...
class PowerSyncRepository {
  ...
  Future<void> initialize({bool offlineMode = false}) async {
    if (!_isInitialized) {
      await _openDatabase();
      shared.logD("powersynced initialized");
      _isInitialized = true;
    }
  }
  ...
  Future<void> _openDatabase() async {
    ...
    if (isLoggedIn()) {
      currentConnector = SupabaseConnector(_db);
